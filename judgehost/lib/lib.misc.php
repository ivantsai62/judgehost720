...
function calcScoreRow(int $cid, int $team, int $prob, bool $updateRankCache = true)
{
    ...
    $result = $DB->q('SELECT result, verified, submittime,
                     ...
                     'AND submittime < c.endtime
                      ORDER BY submittime',
                     $team, $prob, $cid);
    $subtask_result = $DB->q('SELECT subtask,task_point FROM problem WHERE probid = %i',$prob);
    $subtask_exist = $subtask_result->next();
    if(!empty($subtask_exist['subtask']))
    {
        $max_point = 0;
        $total_point = 0;
        $task_point = explode(",",$subtask_exist['task_point']);
        for( $i = 0 ; $i < count($task_point) ; $i++)
        {
            $total_point = $total_point + $task_point[$i];
        }
    }
    // reset vars
    ...
    // for each submission
    //計算所有submission(計算結果到scoreboard上面重丟了幾次有沒有通過等等)
    while ($row = $result->next()) {

        // Contest submit time
        $submittime = calcContestTime((float)$row['submittime'], $cid);
        //檢查是否這個submission有public的judging結果
        ...
        if(!empty($subtask_exist['subtask']))
        {
            ...
            if ($row['result'] == 'wrong-answer') {
                $new_point = 0;
                for ( $i = 0 ; $i < $subtask_exist['subtask'] ; $i++ )
                {
                    $new_point = $new_point + $row['task_result'][$i] * $task_point[$i];
                }
            ...
            $all_task_point = array();
            $all_task_point[0] = $total_point;
            $all_task_point[1] = $max_point;
        }
